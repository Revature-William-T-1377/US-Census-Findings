...
object Main {
  def main(args: Array[String] = Array("g")): Unit = {
    ...
    // Future Analysis
    queries.FutureTestEx.main(Array(""))
    ...
    val headers = session.spark.read.format("csv").option("header","true").load(s"src/main/scala/queries/headers.csv") // File location in hdfs
